...
type ApiColor = (typeof COLORS)[number]["apiValue"];
...
type GameTransaction = {
  ...
  gameType: "COLOR_CHANGER";
  userColor: ApiColor;
  winningColor: ApiColor;
  result: "WIN" | "LOSE";
};

type GameTransactionsResponse = {
  success: boolean;
  message: string;
  data: {
...
export const ColorChanger = () => {
   ...
   * BLUE -> Blue
   */
  const getColorByApiValue = useCallback(
    (value?: string) => {
      return COLORS.find(
        (color) => color.apiValue === value
      );
    },
    []
  );
  ...
  /*
   * Fetch recent game transactions.
   *
   * apiFetch already returns parsed JSON,
   * so DO NOT call response.json().
   */
  const fetchGameTransactions = useCallback(
    async () => {
      try {
        setIsLoadingHistory(true);

        const json: GameTransactionsResponse =
          await apiFetch(
            "/games/gameTransactions?gameName=COLOR_CHANGER",
            {
              method: "GET",
            }
          );
  ...
  const playGame = async () => {
    ...
    try {
      ...
      /*
       * apiFetch already parses JSON.
       */
      const json: BetResponse =
        await apiFetch(
          "/games/bet/color-changer",
          {
            method: "POST",
            body: JSON.stringify({
              color:
                selectedColorData.apiValue,
       ...
       * Convert GREEN -> Green.
       */
      const winningColorData =
        getColorByApiValue(
          data.winningColor
        );

      setResult(
        winningColorData?.name ?? null
      );
